...
Redesign the notes UI to support modular filtering by taller/hora via CascadeFilters, multi-level notes (5 levels), and a clearer create/edit flow, removing dependency on the SidePanel for primary note management while keeping it as a secondary access point.

## Requirements

### Requirement: Notes Filtering by Taller via CascadeFilters
...
### Requirement: Notes Filtering by Hora via CascadeFilters
...
### Requirement: Remove SidePanel Dependency (Primary)
...
### Backend
- **File**: `core/views/portal_docente/notas_view.py` — add filter params (`horario_id`, `ciclo_id`, `nivel`, `alumno_id`) to list endpoint
- **API**: Existing CRUD endpoints gain query parameter filtering and `nivel`/`alumno_id` fields

### Frontend
- **New component**: `CascadeFilters` — shared with AlumnosPage, replaces NotesFilterBar
- **New component**: `NotesInlineForm` — create/edit form embedded in list
- **File**: `src/pages/NotesPage.tsx` — rewritten with CascadeFilters + 5-level notes filtering
- **File**: `src/components/domain/SidePanel.tsx` — notes section as secondary access point (AlumnoProfile context)
- **File**: `src/api/portalDocente.ts` — add filter params to `getNotas()`, add `nivel` and `alumno_id` fields
- **Deleted**: `NotesFilterBar` — replaced by CascadeFilters
